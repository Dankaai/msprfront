...
//------- Début des paramètres type_merchandising -----------
 $erreur_type_merchandising = NULL;
if(strlen($_POST['type_merchandising']) > 256)
{ $erreur_type_merchandising = "Le champs type_merchandising est trop long";
$i++;
}
  if (strlen($_POST['type_merchandising']) < 0)
{ $erreur_type_merchandising= "Le champs type_merchandising est trop court";
$i++;
}
else 
{
  echo($_POST['type_merchandising']);
}
//------- Fin des paramètres type_merchandising -----------
...
//------- Début des paramètres is_accessible -----------
 ...
 if ($_POST['is_accessible'] == NULL)
{ $erreur_is_accessible = "Vous n'avez pas remplie le champs is_accessible";
$i++;
    }
    else {
      echo($_POST['is_accessible']);
    }
...
if($i > 0){
echo $erreur_nom;
echo $erreur_type_merchandising;
...
$query=$pdo->prepare('INSERT INTO boutique (
nom, type_merchandising, is_accessible,  latitude, longitude, heure_debut, heure_fin, information
) VALUES (:nom, :type_merchandising, :is_accessible, :latitude, :longitude, :heure_debut, :heure_fin,  :information)');
$query->bindValue(':nom', $_POST['nom'], PDO::PARAM_STR);
$query->bindValue(':type_merchandising', $_POST['type_merchandising'], PDO::PARAM_STR);
